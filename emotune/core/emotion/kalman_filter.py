...
class KalmanEmotionFilter:
    ...
    def update(self, observation: Dict) -> Tuple[np.ndarray, np.ndarray]:
        """Update step with emotion observation"""
        z = np.array([observation['valence'], observation['arousal']])
        
        # Adaptive measurement noise based on uncertainty
        uncertainty = observation.get('uncertainty', 0.5)
        R = self.R_default * (1.0 + uncertainty * 2.0)  # Scale noise with uncertainty
